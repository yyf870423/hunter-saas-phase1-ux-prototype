...
> 状态：第三轮人类验收反馈已修复，生产构建和全量 `49` 项测试通过，等待人类再次审批。本文只记录阶段二新增组件；基础 Token、按钮、输入、状态、Modal、Toast 和响应式壳层继续以 `stage-1-design-system.md` 为准。
...
9. 执行计划展开后使用单列步骤；计划依据和相关任务默认隐藏，用户按需展开，不能左右并列挤压对话区域。
10. 候选人审核底部使用全宽批量处理区，处理选项必须同时说明结果及影响边界，确认按钮必须表达实际处理结果。
