...
> 状态：第四轮人类验收反馈已修复，生产构建、全量 `49` 项测试和关键截图复核通过，等待人类再次审批。本文只记录阶段二新增组件；基础 Token、按钮、输入、状态、Modal、Toast 和响应式壳层继续以 `stage-1-design-system.md` 为准。
...
9. 执行计划展开后使用单列步骤；“计划步骤 / 计划依据”使用同一区域标签切换，不能使用上下箭头，也不能把依据追加到步骤下方。相关任务继续独立收起，不能与计划左右并列挤压对话区域。
10. 候选人审核底部使用全宽批量处理区。候选人勾选后直接提供“加入联系名单 / 加入岗位储备 / 本轮排除”三个业务动作；不允许先切换处理方式、再点击确认的两段式交互。
