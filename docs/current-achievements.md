...
| 原型阶段三 | 四类业务任务完整流程   | 已审批并冻结 | 客户开发、岗位招聘、公司组织梳理、候选人求职的渐进过程、分支、等待、审核、恢复和结束条件 |
...
2026-09-08 按用户确认收敛为：针对某家或若干家公司整理组织架构、关键岗位与任职人。按公司独立图页保存人才地图，未知任职人保留岗位，身份冲突与未核实汇报关系不得转成确定事实。

批次审核使用“公司与组织”“关键岗位与任职人”“冲突与待补充”三个标签。公司生态、人才流动、联系路径和学术脉络不是本任务的默认交付，不自动加入任何岗位储备。

岗位人才摸排仍属于岗位招聘；审核加入储备时，同一事务更新岗位人才梳理，保留匹配依据、风险、来源和当前流程状态。详见 [本轮交付记录](position-organization-mapping-20260908.md)。
...
3. 公司组织梳理：<https://yyf870423.github.io/hunter-saas-phase1-ux-prototype/#/tasks/mapping-embodied>。专用状态包括 `waiting`、`conflict` 和 `gaps`。
