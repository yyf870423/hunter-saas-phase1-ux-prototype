...
5. 授权模式固定为“仅分析”“执行前确认”“自动执行”，可以在运行过程中切换，只影响尚未执行的动作，不能关闭 Hunter 的结构、安全、权限和写入门禁。
6. 外部等待不显示持续运行假象，也不持续消耗 Agent 用量。页面显示等待对象、渠道、开始时间、最近检查和下一次处理安排，并支持回复或新资料回流后继续。
...
已覆盖信号核验、目标公司判断、公司和联系人草稿、联系方式与证据审核、联系授权、等待外部回复、招聘机会回流，以及未找到可联系对象、权限受限和局部失败等分支。

### 3.2 岗位招聘

已覆盖岗位理解、多渠道找人、候选人补全与去重、人岗匹配、候选人审核、加入岗位储备、单独确认联系范围、等待候选人回复和新简历回流。无候选人通过门禁时解释原因，不自动放宽硬门槛。
...
2. 岗位招聘：<https://yyf870423.github.io/hunter-saas-phase1-ux-prototype/#/workstreams/position-vla>。专用状态包括 `waiting`、`review`、`no-candidate` 和 `candidate-reply`。
