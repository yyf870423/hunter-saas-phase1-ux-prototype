...
| 修改密码           | 个人资料     | Modal、PasswordInput               | 校验后更新                | 原密码错误、强度不足                 |
| 管理活跃会话       | 个人资料     | SessionList、ConfirmModal          | 退出其他会话              | 会话读取失败、操作失败               |
| 修改通知偏好       | 通知         | NotificationMatrix、Toggle         | 保存选择                  | 邮箱未验证、强制项不可关闭、保存失败 |
| 修改默认授权       | 自动化授权   | AuthorizationModal                 | 选择模式并确认            | 自动执行影响确认、保存失败           |
| 查看强制门禁       | 自动化授权   | LockedRuleList                     | 清晰显示始终开启          | 不提供关闭操作                       |
| 连接发件邮箱       | 连接         | StepModal                          | 选择、授权、成功          | 授权取消、连接失败、重试             |
...
| 查看订单记录       | 订阅与用量   | OrderList                          | 查看和下载收据            | 空状态、下载失败                     |
...
| 注销账号           | 数据与隐私   | DangerModal                        | 影响确认后提交            | 有效订阅阻塞                         |
...
| 邮箱未验证 | `/settings/notifications?state=limited` | 邮件通知不可选，站内通知可用 |
...
| 删除阻塞   | `/settings/data-privacy?state=limited`  | 阻塞原因和可处理动作清楚     |
