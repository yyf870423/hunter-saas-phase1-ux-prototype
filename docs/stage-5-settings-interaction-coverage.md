...
2026-09-07 新增“导航”页：资产显隐草稿、保存、恢复默认、刷新及同源页面同步、全部平铺/全部收起、读取与保存失败、权限受限、移动端其他返回。具体组件与验收状态见 `asset-navigation-preferences.md`；未保存的导航草稿离开或刷新后丢弃，不影响已保存导航。

同日扩展知识图谱快捷类型：与资产显隐共同保存，允许全部关闭；父入口隐藏时不保留孤立子项，重新平铺恢复原类型选择。覆盖见 `graph-types-navigation-prototype.md`。
...
| 查看套餐与用量     | 订阅与用量     | MetricDonut、UsageBreakdown        | 查看明细                   | 用量明细加载失败                                 |
