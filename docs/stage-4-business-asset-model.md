...
> 2026-09-07 知识图谱类型补充：每份图谱具有一个必填 `typeId`，人才地图替代组织架构，保留其余七类；图谱归类独立于内部图页和节点类型。新建、编辑、筛选、回收恢复与快捷导航共用目录，原型实现和持久化边界见 `graph-types-navigation-prototype.md`。

> 2026-09-07 最新确认：移除独立关联业务区块及配置，保留只读关联任务入口。任务依其持有的资产类型和稳定 ID 引用反向展示，不按公司或姓名推断；联系人还须匹配公司 ID。用户任务与资产 AI 处理记录分开，底层事实关系和来源不删除。页面范围以 `asset-business-sections-removal.md` 为准。
...
阶段一共有七类独立正式业务资产，联系人是公司的可引用子资产：

| 资产                 | 保存的长期事实                                                     | 不应混入的内容                                             |
| -------------------- | ------------------------------------------------------------------ | ---------------------------------------------------------- |
| 公司                 | 公司资料、招聘特征、任职人才、招聘业务和人才版图关联               | 一次公司调研日志、未经确认的公司猜测、单一客户状态         |
| 公司联系人（子资产） | 某家公司下的联系人身份、角色、联系方式、备注和沟通记录             | 无所属公司的人物线索、其他公司下的沟通内容、完整候选人简历 |
| 招聘机会             | 用户决定记录并持续跟进的潜在招聘需求，明确区分事实、推断与待确认项 | 无明确公司或可解释依据的宽泛信号、完整岗位招聘流程         |
...
| 联系人在某公司下的业务资料         | 公司联系人子记录，所属公司必填 | 公司联系人 Tab 和嵌套详情读取同一记录；跨公司独立维护 |
...
6. **人工或自动确认**：对话中单个资产入库和修改均需用户明确的是/否/建议，自动执行不跳过；其他已授权处理不关闭强制门禁，冲突和身份不确定必须暂停。单资产确认不使用专用审核页，实际写入仍由受控业务命令执行。
...
| 公司联系人是公司内部子记录                         | 保留公司子资产归属，同时提供完整资料、沟通、文件和联系路径详情 | 同一自然人在不同公司分别建记录，跨业务引用带所属公司 |
...
3. 公司专属联系人记录与候选人同人身份是否清楚；各公司的业务记录不能自动合并。
