...
import { candidates as assetCandidates, matchResults, positionDetail, positions } from "./data";
...
const legacyPipelineCandidateSeed = {
  zhaoxingyu: {
    id: "zhaoxingyu",
    name: "赵星羽",
    company: "星澜机器人",
...
function CandidatePipeline({ position }) {
  const incoming = (position?.pipeline || []).map((record) => {
    const person = record.candidate || assetCandidates.find((item) => item.id === record.candidateId);
    const legacyId = record.candidateId.replace(/^candidate-/, "");
    return { ...record, id: legacyPipelineCandidateSeed[legacyId] ? legacyId : record.candidateId, person };
  }).filter((record) => record.person);
  const pipelineCandidateSeed = { ...legacyPipelineCandidateSeed, ...Object.fromEntries(incoming.map((record) => [record.id, { ...record.person, id: record.id, title: record.person.title || record.person.role, enteredAt: "本轮审核", note: record.person.risk || "待联系核实" }])) };
  const initialStages = initialPipelineStages.map((stage) => ({ ...stage,
    people: [...stage.people.filter((id) => !incoming.some((record) => record.id === id)), ...incoming.filter((record) => record.stage === (stage.id === "salary" ? "offer" : stage.id)).map((record) => record.id)] }));
  const notify = useToast();
  ...
  const moveCandidate = (note) => {
    ...
    const stored = incoming.find((record) => record.id === pendingMove.personId);
    if (stored) {
      try { runOpportunityCommand("pipeline.move", { positionId: position.id, candidateId: stored.candidateId, stage: pendingMove.to === "salary" ? "offer" : pendingMove.to, note }); }
  ...
  return (
    <div className="s4-detail-stack">
      ...
      />
      <StageMoveNoteModal
        candidates={pipelineCandidateSeed}
        move={pendingMove}
...
function StageMoveNoteModal({ move, close, onSave, candidates }) {
  const [note, setNote] = useState("");
  return (
    <Modal
      open={Boolean(move)}
      close={close}
      title={`移动 ${candidates[move?.personId]?.name || "候选人"}`}
...
function LegacyPositionDetailPage() {
  ...
  return (
    <div className="s4-detail-page">
      ...
      <DetailTabs
        tabs={tabs.map((entry) => entry.value === "talent-map" ? { ...entry, count: item.talentMap?.rows.length || 0 } : entry.value === "pipeline" ? { ...entry, count: new Set([...initialPipelineStages.flatMap((stage) => stage.people), ...item.pipeline.map((record) => record.candidateId.replace(/^candidate-/, ""))]).size } : entry)}
        value={tab}
        onChange={(value) =>
          updateQuery({
            tab: value,
      ...
      {tab === "pipeline" ? <CandidatePipeline position={item} /> : null}
      {tab === "matching" ? <MatchingResults /> : null}
      {tab === "talent-map" ? <PositionTalentMap position={item} /> : null}
