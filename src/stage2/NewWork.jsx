...
function OutcomeReply({ outcome, prompt }) {
  if (outcome === "mainline") {
    return (
      <HunterReply
        markdown={`这项任务需要持续汇总系统候选人、专题图谱、公开资料和用户主动上传的简历，并在审核、邮件联系和等待回复后继续推进。我会保留完整任务上下文并持续更新计划；新资料上传后会先经过身份判断、查重和匹配，再进入审核。
...
> 正在创建任务，并保留当前输入、附件和授权方式。`}
  ...
  if (outcome === "task") {
    return (
      <HunterReply
        markdown={`这是一项范围有限、交付明确的核验任务。我会直接执行并交付结论；核验完成后，结果会回到对应的专题图谱，不会自动合并人物。
