...
export function WorksPage() {
  ...
  const [legacyRows, setRows] = useState(workItems);
  const lifecycle = useOpportunityState();
  const rows = useMemo(() => mergeLifecycleTasks(legacyRows, lifecycle), [legacyRows, lifecycle]);
        ...
        footer={
          <>
            ...
            <Button
              ...
              onClick={() => {
                try {
                if (lifecycle.tasks.some((task) => task.id === deleteWork.id)) runOpportunityCommand("task.recycle", { id: deleteWork.id });
                setRows((items) =>
...
function PositionCreationTask({ taskId }) {
  ...
  useEffect(() => {
    if (task) return;
    try {
      runOpportunityCommand("task.create", { id: taskId, kind: "position-create", title: "整理岗位资料", authMode: "confirm", allowedResults: ["position"],
        prompt: sessionStorage.getItem("hunter-new-task-prompt") || "公司：星澜机器人\n岗位名称：具身智能 VLA 算法负责人\n工作地点：北京\n岗位职责\n负责 VLA 算法研发与真实机器人部署，管理 8 人以上研发团队。\n任职要求\n具有 VLA 研发、真机部署与团队管理经验。" });
    } catch (failure) { setError(failure.message); }
  }, [taskId, task]);
  return task ? <OpportunityTaskWorkspace taskId={taskId} /> : <HunterReply markdown={error || "正在整理岗位资料。"} />;
}
